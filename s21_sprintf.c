...
void handle_value(flags f, char *buff, va_list va) {
    ...
    if (f.specifier == 'G' || f.specifier == 'E' || f.specifier == 'X') to_upper(buff);
}
...
void unsigned_num_to_string(uint64_t val, char *ret, int base) {
    ...
    for (; val && idx; --idx, val /= base) buf[idx] = "0123456789abcdef"[val % base];
...
void format_wchar(char *buff, wchar_t w_c) {
    ...
    // Проверка на ошибку в wcstombs
    if (len == (s21_size_t)-1) {
        return;
    }

    // // Добавление пробелов слева
    // if ((s21_size_t)f.width > len) {
    //     for (s21_size_t i = len; i < (s21_size_t)f.width; i++) {
    //         buff[i] = ' ';
    //     }
    // }
}
...
void double_to_string(long double val, char *ret, flags f) {
    ...
    if ((f.is_precision_set && f.precision != 0) || (int)r || (!f.is_precision_set && val == 0) ||

...
START_TEST(many_flags_many_ints) {
    ...
    int val = 69;
    ck_assert_int_eq(s21_sprintf(str1, format, 5, val, -10431, 13, 5311, 0, -581813581),
...
START_TEST(null_ptr) {
    ...
    char *format = "%s";
    ck_assert_int_eq(s21_sprintf(str1, format, "(null)"), sprintf(str2, format, "(null)"));
...
START_TEST(float_many) {
    ...
    float val = 123.456f, val1 = -123.456f, val2 = 789.0f, val3 = -789.0f, val4 = 0.0f, val5 = 1234567.0f;
...
START_TEST(test_many_hex_lower) {
    ...
    ck_assert_int_eq(s21_sprintf(str1, "%x%x%x%x%x", 12340987, 135, 0, -1230, -123213123),
...
START_TEST(test_many_hex_lower_with_alignment) {
    ...
    ck_assert_int_eq(s21_sprintf(str1, "%2x%-7x%9x%11x%0x", 12340987, 100, 0, 1, -666999),
...
START_TEST(test_many_hex_lower_with_hashtag_and_alignm) {
    ...
    ck_assert_int_eq(s21_sprintf(str1, "%#x%#x%3x%#32x%#-1x", 87, 1222224535, -13, -0, 123213123),
...
START_TEST(test_many_hex_lower_with_width_star_and_align_and_hashtag) {
    ...
    ck_assert_int_eq(s21_sprintf(str1, "%#*x%*x%-12x%3x%*x", 9, 127312897, 0, -1, 199, -123978, 3, -1251),
                     sprintf(str2, "%#*x%*x%-12x%3x%*x", 9, 127312897, -0, -1, 199, -123978, 3, -1251));
    ck_assert_str_eq(str1, str2);
}
END_TEST
...
START_TEST(test_many_hex_lower_with_precision_and_other) {
    char str1[BUFF_SIZE];
    char str2[BUFF_SIZE];
    int a = s21_sprintf(str1, "%#3.*x%#3x%-7.*x%#-1.8x%4.3x%#2.15x%*.*x%*.1x%3x%-1x", 3, 126714, 4444444, 0,
                        6423235, 0, 666, 999, 13, 5, 419, 9, 41, -41, 33);
    int b = sprintf(str2, "%#3.*x%#3x%-7.*x%#-1.8x%4.3x%#2.15x%*.*x%*.1x%3x%-1x", 3, 126714, 4444444, 0,
                    6423235, 0, 666, 999, 13, 5, 419, 9, 41, -41, 33);
    ck_assert_int_eq(a, b);
    ck_assert_str_eq(str1, str2);
}
...
START_TEST(test_many_hex_upper_with_precision_and_other) {
    ...
    ck_assert_int_eq(s21_sprintf(str1, "%#3.*X%#3x%-7.*X%#-1.8X%4.3X%#2.15x%*.*X%*.1X%3X%-1X", 3, -126714,
                                 4444444, 0, 6423235, 0, 666, 999, 13, 5, -419, 9, 41, -41, 33),
                     sprintf(str2, "%#3.*X%#3x%-7.*X%#-1.8X%4.3X%#2.15x%*.*X%*.1X%3X%-1X", 3, -126714,
                             4444444, 0, 6423235, 0, 666, 999, 13, 5, -419, 9, 41, -41, 33));
    ck_assert_str_eq(str1, str2);
}
END_TEST
...
START_TEST(test_sprintf12) {
    ...
    ck_assert_int_eq(s21_sprintf(str1, format, 66666666666, 5555555555, 44444444444, k),
...
START_TEST(test_sprintf15) {
    ...
    ck_assert_int_eq(s21_sprintf(str1, format1, 1), sprintf(str2, format1, 1));
    ...
    ck_assert_int_eq(s21_sprintf(str3, format2, 222), sprintf(str4, format2, 222));
    ...
    ck_assert_int_eq(s21_sprintf(str5, format3, 33333333333), sprintf(str6, format3, 33333333333));
...
START_TEST(test_sprintf25) {
    ...
    ck_assert_int_eq(s21_ret, sprintf_ret);
    ck_assert_str_eq(str1, "121.123");  // Ожидаемое значение с тремя знаками после запятой
...
START_TEST(test_sprintf26) {
    ...
    ck_assert_int_eq(s21_sprintf(str1, "%s", format), sprintf(str2, "%s", format));
...
START_TEST(test_sprintf29) {
    ...
    ck_assert_int_eq(s21_ret, sprintf_ret);
    ck_assert_str_eq(str1, "111.111");  // Ожидаемое значение с тремя знаками после запятой
...
START_TEST(g_trailing_zero) {
    // char str1[1000], str2[1000];
    // const char *format = "%Lg"; // Формат для long double
    // long double val = 123.45;
    // int s21_ret = s21_sprintf(str1, format, val);
    // int sprintf_ret = sprintf(str2, format, val);
...
START_TEST(g_many) {
    ...
    ck_assert_int_eq(s21_sprintf(str1, format2, hex1, hex1), sprintf(str2, format2, hex1, hex1));
    ck_assert_str_eq(str1, str2);

    ck_assert_int_eq(s21_sprintf(str1, format3, hex1), sprintf(str2, format3, hex1));
    ck_assert_str_eq(str1, str2);
}
END_TEST
